...
cmake -B build/rel -DCMAKE_BUILD_TYPE=Release -DCMAKE_EXPORT_COMPILE_COMMANDS=ON $CLAP_OPTS
...
if which emcmake 1>/dev/null 2>&1; then
	emcmake cmake -B build/emrel -DCMAKE_BUILD_TYPE=Release -DCMAKE_EXPORT_COMPILE_COMMANDS=ON $CLAP_OPTS "$www_dir"
