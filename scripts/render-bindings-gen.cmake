...
function(emit_common_header out_var)
    ...
    foreach (backend ${backends})
        ...
        if (backend STREQUAL "WGPU")
            append_line(c "#define ${backend}_BINDING_UBO_BASE ${slot}")
        endif ()
...
# ---------------------------------------------------------------------------
# DT_* type to GLSL type conversion
# ---------------------------------------------------------------------------
# The mapping is: strip the DT_ prefix and lowercase (DT_VEC3 -> vec3, etc).
# This matches the canonical names in core/datatypes.c:type_name[].
# UBO entries with a "glsl_type" override in JSON take precedence (e.g.
# DT_INT -> bool for boolean uniforms stored as int on the CPU side).

function(dt_to_glsl out_var dt_type)
    string(REGEX REPLACE "^DT_" "" glsl "${dt_type}")
    string(TOLOWER "${glsl}" glsl)
    set(${out_var} "${glsl}" PARENT_SCOPE)
endfunction()

# ---------------------------------------------------------------------------
# ubo_*.glsl -- GLSL shader UBO layout declarations
# ---------------------------------------------------------------------------
# One file per UBO block, suitable for #include from .vert/.frag shaders.
# Uses UBO_BINDING_* macros from the backend alias headers so the layout
# is renderer-agnostic at the source level.

function(emit_ubo_glsl ubo_dir)
    file(MAKE_DIRECTORY "${ubo_dir}")
    foreach (record ${ubo_records})
        string(REPLACE "|" ";" f "${record}")
        list(GET f 0 ubo_idx)
        list(GET f 1 ubo_name)
        string(TOUPPER "${ubo_name}" ubo_upper)

        set(c "/* Generated by scripts/render-bindings-gen.cmake. */\n")
        append_line(c "#ifndef SHADERS_UBO_${ubo_upper}_GLSL")
        append_line(c "#define SHADERS_UBO_${ubo_upper}_GLSL")
        append_line(c "")
        append_line(c "layout (std140, binding = UBO_BINDING_${ubo_name}) uniform ${ubo_name} {")
        ...
        foreach (u RANGE 0 ${u_last})
            json_get(u_name "${json}" ubos ${ubo_idx} uniforms ${u} name)
            json_get(u_type "${json}" ubos ${ubo_idx} uniforms ${u} type)
            json_get_optional(u_glsl has_glsl "${json}" ubos ${ubo_idx} uniforms ${u} glsl_type)
            json_get_optional(u_arr has_arr "${json}" ubos ${ubo_idx} uniforms ${u} array_size)

            if (has_glsl)
                set(glsl_type "${u_glsl}")
            else ()
                dt_to_glsl(glsl_type "${u_type}")
            endif ()

            if (has_arr)
                append_line(c "    ${glsl_type} ${u_name}[${u_arr}];")
            else ()
                append_line(c "    ${glsl_type} ${u_name};")
            endif ()
        endforeach ()

        append_line(c "};")
        append_line(c "")
        append_line(c "#endif /* SHADERS_UBO_${ubo_upper}_GLSL */")

        write_if_different("${ubo_dir}/ubo_${ubo_name}.glsl" "${c}")
    endforeach ()
endfunction()
