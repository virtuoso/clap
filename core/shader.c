...
static const struct shader_var_desc shader_var_desc[] = {
    ...
    SHADER_ARR(UNIFORM_LIGHT_CUTOFF,        "light_cutoff",         DT_FLOAT, LIGHTS_MAX),
    ...
    SHADER_VAR(UNIFORM_USE_NOISE_NORMALS,   "use_noise_normals",    DT_INT),
    SHADER_VAR(UNIFORM_NOISE_NORMALS_AMP,   "noise_normals_amp",    DT_FLOAT),
    SHADER_VAR(UNIFORM_NOISE_NORMALS_SCALE, "noise_normals_scale",  DT_FLOAT),
    SHADER_VAR(UNIFORM_USE_NOISE_EMISSION,  "use_noise_emission",   DT_INT),
    SHADER_VAR(UNIFORM_USE_3D_FOG,          "use_3d_fog",           DT_INT),
    SHADER_VAR(UNIFORM_FOG_3D_AMP,          "fog_3d_amp",           DT_FLOAT),
    SHADER_VAR(UNIFORM_FOG_3D_SCALE,        "fog_3d_scale",         DT_FLOAT),
    ...
    SHADER_VAR(UNIFORM_NR_CASCADES,         "nr_cascades",          DT_INT),
...
static cerr shader_prog_make(struct ref *ref, void *_opts)
{
    ...
    /*
     * On GLES/WebGL, this is not optional, because UBOs there don't have
     * binding points in the shaders, they need to be explicitly written
     * (shader_uniform_buffer_bind()), otherwise their binding points
     * will be assigned in the order in which they appear, which is not
     * useful.
     */
#if (defined(CONFIG_RENDERER_OPENGL) && defined(CONFIG_GLES)) || !defined(CONFIG_FINAL)
    for (int i = 0; i < array_size(shader_var_block_desc); i++) {
...
#endif /* CONFIG_GLES && !CONFIG_FINAL */
