...
static inline size_t type_size(data_type type)
{
...
/* Get Metal storage size + padding (stride) for a given type */
static inline size_t type_storage_size(data_type type)
{
    size_t storage_size = type_size(type);
    if (!storage_size) {
        clap_unreachable();
        return 0;
    }
    ...
    switch (type) {
        case DT_MAT2:
            storage_size = 2 * type_size(DT_VEC4); /* vec2 padded to vec4 */
            break;
        case DT_MAT3:
            storage_size = 3 * type_size(DT_VEC4); /* vec3 padded to vec4 */
            break;
        ...
        default:
             ...
             * uniform_buffer_set() handles the offset alignment.
             */
            break;
    }
...
// static inline void buffer_cache_clear(buffer_cache_t *bc)
// {
//     memset(bc, 0, sizeof(*bc));
// }

// static inline bool buffer_cache_insert(buffer_cache_t *bc, size_t slot, void *data, size_t len)
// {
//     if (bc->slot[slot].data == data && bc->slot[slot].len == len)   return false;

//     bc->slot[slot].data = data;
//     bc->slot[slot].len  = len;
...
static mtl_buffer_t mtl_buffer_new(renderer_t *r, size_t size)
{
    return [r->device newBufferWithLength:(NSUInteger)size options:MTLResourceStorageModeShared];
}
...
static cerr buffer_make(struct ref *ref, void *_opts)
{
    ...
    if (!opts->main) {
        buf->buf = mtl_buffer_new(buf->renderer, opts->size);
        if (!buf->buf)  return CERR_NOMEM;

        void *buf_data = [buf->buf contents];
        memcpy(buf_data, opts->data, opts->size);
...
//
// fbo_t (render pass desc -> command encoder) "where to render" -> cmd_encoder    ||  create: fbo_new(): create render pass desc
// - depth/color attachments: textures, load/store actions                         ||  use:    fbo_prepare(): create cmd_encoder from desc, renderer->fbo = fbo
//                                                                                 ||
// draw_control_t (pipeline state) "how to render (to fbo_t/destination)"          ||  create: draw_control_make(renderer, shader, fbo): NOPE: 1) different fbos; 2) not every fbo
// - blending, rasterization, samples                                              ||  ???:    from shader reflection? NOPE: pixelFormat is fbo-dependant
// - depth/color attachments: pixel formats, blending                              ||
// - shader functions                                                              ||  use:    shader_use(): pick draw_control_t based on renderer->fbo
// - vertex attribute layout                                                       ||                    OR: draw_control_t picks PSO based on renderer->fbo
//                                                                                 ||  XXX:    shader_use(): called without bound fbo in e.g. model3d_make(): can be a NOP there
//                                                                                 ||  ???:    hash { fbo, blending } -> PSO /* fbo::nr_samples */
//                                                                                 ||  BLERGH: internal fbo::id, shader::id -- trivially hashable
//                                                                                 ||          (except fbos come and go (ui_printf()))
//                                                                                 ||  renderer::hash{ fbo, shader, blending } -> PSO || shader::hash{ fbo, blending } -> PSO
// buffer_t (MTLBuffer, vertex data) "what to render: mesh"                        ||
//                                                                                 ||
// texture_t (texture, sampler) "what to render: textures (sources)"               ||
//                                                                                 ||
// viewport (??)                                                                   ||
//                                                                                 ||
// cull_face                                                                       ||
...
{
    uintptr_t x = fbo->id ^ (shader->id << 4);
    // if (blend)  x ^= 0x8;//{ unsigned int pos = flsl(x); x ^= 1ull << pos; }
    // dbg("### fbo %d shader %d hash %lx\n", fbo->id, shader->id, x);
    return x;
}
...
static cerr draw_control_make(struct ref *ref, void *_opts)
{
    ...
    for (size_t pl = 0; pl < array_size(dc->pipeline); pl++) {
        ...
        if (fbo) {
            ...
            bool use_depth = fbo->attachment_config.depth_texture;
...
static cerr texture_make(struct ref *ref, void *_opts)
{
    ...
    sdesc.tAddressMode          = MTLSamplerAddressModeMirrorClampToEdge;//mtl_wrap_mode(opts->wrap);
    sdesc.rAddressMode          = mtl_wrap_mode(opts->wrap);
    sdesc.normalizedCoordinates = YES;
    sdesc.compareFunction       = MTLCompareFunctionAlways;

    tex->sampler = [renderer->device newSamplerStateWithDescriptor:sdesc];
...
#ifndef CONFIG_FINAL
/* XXX: vvv common with all renderers */
...
texid_t texture_id(struct texture *tex)
{
    ...
    // return [tex->texture gpuResourceID]._impl;
...
static void mtl_cmd_buffer(renderer_t *r)
{
...
void fbo_prepare(fbo_t *fbo)
{
    ...
    // MTLViewport viewport = { 0, 0, fbo->width, fbo->height };
...
must_check cresp(fbo_t) _fbo_new(const fbo_init_options *opts)
{
    ...
    if (fbo->attachment_config.color_textures)
        CERR_RET_T(fbo_textures_init(fbo, fbo->attachment_config), fbo_t);

    // fbo->renderer->fbo = fbo;
...
{
    CERR_RET(ref_embed(uniform_buffer, ubo, .renderer = r, .binding = binding, .name = name), return __cerr);
...
cerr uniform_buffer_data_alloc(uniform_buffer_t *ubo, size_t size)
{
    ...
    ubo->buf = mtl_buffer_new(ubo->renderer, size);
    [ubo->buf setLabel:[NSString stringWithUTF8String:ubo->name]];

    ubo->data = [ubo->buf contents];
    ubo->size = size;
    memset(ubo->data, 0, ubo->size);
...
{
    // if (ubo->dirty) {
    //     void *ubo_data = [ubo->buf contents];
    //     memcpy(ubo_data, ubo->data, ubo->size);
    ...
    if (binding_points->stages & (1 << SHADER_STAGE_VERTEX)) {
        if (r->vbuffer_cache[ubo->binding] != ubo->data || ubo->dirty) {
            [cmd_encoder(r) setVertexBytes:ubo->data length:ubo->size atIndex:ubo->binding];
            r->vbuffer_cache[ubo->binding] = ubo->data;
        }
    }
    if (binding_points->stages & (1 << SHADER_STAGE_FRAGMENT)) {
        if (r->fbuffer_cache[ubo->binding] != ubo->data || ubo->dirty) {
            [cmd_encoder(r) setFragmentBytes:ubo->data length:ubo->size atIndex:ubo->binding];
            r->fbuffer_cache[ubo->binding] = ubo->data;
        }
    }
...
{
    size_t elem_size = type_size(type);            /* C ABI element size */
    ...
    for (unsigned int i = 0; i < count; i++) {
        if (value) {
            ...
            if (type == DT_MAT2 || type == DT_MAT3) {
                ...
                size_t row_size = type_size(is_mat3 ? DT_VEC3 : DT_VEC2);
...
uniform_t shader_uniform(shader_t *shader, const char *name)
{
    ...
    // return uniform >= ATTR_MAX && uniform < UNIFORM_TEX_MAX;
}
...
void _renderer_init(renderer_t *r, const renderer_init_options *opts)
{
    ...
    r->cull_mode = MTLCullModeBack;
    r->depth_func = DEPTH_FN_LESS;
    r->clear_depth = 1.0;
...
void renderer_frame_begin(renderer_t *r)
{
    ...
    if (!r->screen_fbo)
        r->screen_fbo = CRES_RET(
            fbo_new(
                // .renderer   = r,
                .color_format       = (texture_format[]) { mtl_texture_format_from_pixel_format(r->layer.pixelFormat) },
    ...
    // MTLViewport viewport = { 0, 0, r->width, r->height };
    // [r->cmd_encoder setViewport:viewport];
}
                   ...
                   data_type idx_type, unsigned int nr_instances)
{
    ...
    size_t _idx_count = index->size / comp_size[idx_type];
