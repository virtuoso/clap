...
static int light_handle_input(struct message *m, void *data)
{
...
void light_init(struct light *light)
{
    ...
    subscribe(MT_INPUT, light_handle_input, light);
#ifndef CONFIG_FINAL
    ui_debug_set_hover(light_hover, light);
#endif /* CONFIG_FINAL */
}

void light_done(struct light *light)
{
...
#ifndef CONFIG_FINAL
void light_draw(struct light *light)
{
    /* Here we go */
    for (unsigned int idx = 0; idx < light->nr_lights; idx++) {
